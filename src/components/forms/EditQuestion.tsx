...
export default function EditQuestionForm(props:{
  ...
  const QueryCategory = useQuery({
    ...
    const MutateAdd = useMutation({
      mutationFn:async ()=>{
        const response = await axios.put(`${baseurl}/questions/edit/${props.id}`,form.getValues(),{
          headers:{
            "Authorization":`${localStorage.getItem("token")}`
          }
        })
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
              <select value={form.getValues("categoryid")}  onChange={(e)=>{
         ...
         <FormField 
          ...
          render={() => (
            <FormItem>
              ...
              <FormControl>
                <select value={form.getValues("difficulty")} className="w-44 border-2 p-2  rounded-lg block" onChange={(e)=>{
