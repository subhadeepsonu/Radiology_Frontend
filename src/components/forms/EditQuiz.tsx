...
export default function EditQuiz(props:{
  ...
  const MutateQuiz = useMutation({
    mutationFn:async ()=>{
      const response = await axios.put(`${baseurl}/quiz/edit/${props.id}`,form.getValues(),{
        headers:{
          "Authorization":`${localStorage.getItem("token")}`
        }
      })
        ...
        <FormField 
          ...
          render={() => (
            <FormItem>
              ...
              <FormControl>
                <select value={form.getValues("difficulty")} className="w-72 border-2 p-2  rounded-lg block" onChange={(e)=>{
