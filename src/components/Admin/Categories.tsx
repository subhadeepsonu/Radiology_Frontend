...
export default function AdminCategoryPage(){
  ...
  if(QueryCategory.data.data.length>0){
    return <div className="h-fit w-full flex flex-col justify-start items-center my-10">
        <p className="w-full px-5 font-semibold text-4xl pl-24 mb-2">Categories</p>
        <div className="grid grid-cols-4 gap-5">
           {QueryCategory.data.data.map((category:any)=>{
            return <CategoryCard id={category.id} imgurl={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTc9APxkj0xClmrU3PpMZglHQkx446nQPG6lA&s"} name={category.name} questions={2}></CategoryCard>
           })}
        </div>
    </div>
  }
